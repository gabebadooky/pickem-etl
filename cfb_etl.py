import extract.espn.endpoint_data as e
...
def load_game_data(game_data: dict):
...
def load_team_data(team_data: dict):
...
for week in range(weeks):
    week += 1
    games = get_all_games_in_week(week)
    for game in games:
        game_dict = e.get_scoreboard_data(game)
        distinct_away_teams.add(game_dict['away_team_id'])
        load_game_data(game_dict)

# Extract and Load Team related data
for team in distinct_away_teams:
    team_dict = e.get_team_data(team)
    load_team_data(team_dict)
